...
RegisterCommand("plakasok", function()
	...
	if closestDistance <= 5.0 then
		local currentPlate = GetVehicleNumberPlateText(closestVehicle)

		LicencePlate.Index = GetVehicleNumberPlateTextIndex(closestVehicle)
		
		LicencePlate.Number = GetVehicleNumberPlateText(closestVehicle)
		
		SetVehicleNumberPlateText(closestVehicle, "")
...
RegisterCommand("getplates", function()
	for k, v in pairs(SYSTEM.UsedPlates) do
		print(plate)
	end
end)